...
// default) and the process.exit codes become return values. Note: the
// common.ts helpers used here (resolveFees, sendMulticallBatched and its
// pendingTxGuard, the uncaught-error handler) still print to the console
// directly — the shell can revisit that later.
//
...
export async function runRevealFlow(opts: RevealFlowOpts): Promise<number> {
    ...
    if (skipped.length > 0) {
        ...
        for (const s of skipped) out.log(`  ⚠ ${s}`)
    }
    if (datas.length === 0) {
        out.log('\nNothing to reveal.')
        return 0
